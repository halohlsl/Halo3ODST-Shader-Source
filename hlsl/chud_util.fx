...
float4 chud_virtual_to_screen(float3 virtual_position)
{
	...
	{
		screen = float4(chud_transform_2d(transformed_position_scaled), 0.5, 0.5);
	}
	else
	{
		screen = float4(chud_transform(transformed_position_scaled), 0.5, 0.5);
	}
	screen.xy *= chud_widget_mirror.zw;
	return screen;
}
