...
PARAM_STRUCT(s_render_state, g_render_state) PARAM_DX9_REGISTER(c70);
...
float3x3 surface_basis(float2x3 plane_basis, float2 plane_offset, float curvature)
{
	...
	{
		basis[2]+= safe_normalize(tan(curvature)*mul(plane_offset, plane_basis));
...
float4 sample_diffuse(float2 texcoord_sprite, float2 texcoord_billboard, float palette_v, float external_alpha)
{
	...
	IF_CATEGORY_OPTION(albedo, palettized_plasma)
	{
		...
		float noise_b=	sample2D(base_map2,		texcoord_sprite * base_map2_xform.xy	+ base_map2_xform.zw).r;
